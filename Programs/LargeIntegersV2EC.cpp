...
bool subtractNumbers(int number1[], int number2[], int result[]){
    int subon = 0;
    for (int i = 0; i < 25; i++)
    {
        result[i] = number1[i] - number2[i] - subon;
        if(result[i] < 0){
            subon = abs(floor(result[i]/10.0));
    ...
    while(value1 != "0" && sign != '%' && value2 != "0"){
        ...
        if(sign == '+' && value1.length()<=25 && value2.length()<=25){
            ...
            if(addNumbers(array1, array2, result))
                ...
                cout << "overflow";
            cout << endl << endl;
        }else if(sign == '-' && value1.length()<=25 && value2.length()<=25){
            convertToArray(value1, array1);
            convertToArray(value2, array2);
            outputNumber(array1);
            cout << " - ";
            outputNumber(array2);
            cout << " = ";
            if(subtractNumbers(array1, array2, result))
                outputNumber(result);
            else
